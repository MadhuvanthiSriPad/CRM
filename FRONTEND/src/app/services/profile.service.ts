...
export class ProfileService {
  ...
  addProfile(title: string, content: string, imgpath: File) {
    ...
    this.http
      ...
      .subscribe(responseData => {
        ...
        this.err.next(null)
        this.saveProfileData(responseData.post)
  ...
  getProfile() {
     this.http.post<{ profile: any, message: string }>
    (BACKEND_URL + "/test2",{}).subscribe(profile => {
  ...
  getProfileByCreatorId() {
    return this.http.post<{ profile: any, message: string }>
    (BACKEND_URL+"/test2",{})
  }
